...
| Testing | Partially implemented | Zero-dependency test framework with 17 suites (708 assertions) — router, plugin, migration, auth, mailer, remember_me, forgot_password, email_verification, registration, two_factor, organization, organization_runtime, smtp, smtp_settings, version, messenger, telico. CRUD coverage for users/groups/permissions/tokens complete. Mailer + SMTP foundation implemented. Two-factor auth includes regression test for missing-schema degradation. VersionProvider tests cover kernel/app version resolution. Messenger + Telico tests cover transport interface, message immutability, and API client validation. See docs/developer/testing.md |
