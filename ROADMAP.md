...
| Testing | Partially implemented | Zero-dependency test framework with 16 suites (665 assertions) — router, plugin, migration, auth, mailer, remember_me, forgot_password, email_verification, registration, two_factor, organization, organization_runtime, smtp, version, messenger, telico. CRUD coverage for users/groups/permissions/tokens complete. Mailer + SMTP foundation implemented. Two-factor auth includes regression test for missing-schema degradation. VersionProvider tests cover kernel/app version resolution. Messenger + Telico tests cover transport interface, message immutability, and API client validation. See docs/developer/testing.md |
...
| SMTP/Telico settings | Fixed | Bootstrap hooks now register correctly — plugin autoloader updated to handle `Plugins\` namespace (used by SMTP/Telico plugins). Both plugins enabled by default (plugin.json). Settings appear in /admin/settings. |
